...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::IcaBankSend { amount, denom, to_address } => {
            execute::ica_bank_send(deps, env, info, amount, denom, to_address)
        }
    }
}
    ...
    pub fn ica_bank_send(
        ...
        let ica_packet = match ica_info.encoding {
            ...
            TxEncoding::Proto3Json => {
                let amount_u128: u128 = amount.parse().map_err(|_| ContractError::UnknownDataType(amount))?;
                let predefined_json_message = ExampleCosmosMessages::MsgSend {
                    from_address: ica_info.ica_address,
                    to_address,
                    amount: coins(amount_u128, denom),
                }
                .to_string();
